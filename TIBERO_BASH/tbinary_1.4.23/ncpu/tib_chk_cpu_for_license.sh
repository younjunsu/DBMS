...
#  V1.23  : 여한나  : 20140915  : TIBERO 전용으로 수정,NCPU 실행경로(NCPUDIR)OS별 지정(solaris 제외)#
...
linux_check() #@1 : $# @2 : 1bAX5G4B argc @3 : Usage descrption @4 : Example descrption
{
        ...
        #cat /proc/cpuinfo
        #uname -a
        NCPUDIR=`uname -s`"-K"`uname -r |cut -d. -f1-2`"_x"`uname -p | cut -d_ -f2`
...
{
        NCPUDIR=`uname -s`"_"`uname -m`"_"`getconf KERNEL_BITS`
...
{
        NCPUDIR_b=`uname -s`"_"`uname -p| cut -c 1-5`"_"`prtconf | grep Kernel | cut -d : -f 2 | cut -c 2-3`
        NCPUDIR=`echo $NCPUDIR_b | tr '[a-z]' '[A-Z]'`
        __GET_PRO_EDITION 
