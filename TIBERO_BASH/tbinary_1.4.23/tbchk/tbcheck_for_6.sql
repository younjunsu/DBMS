...
/* Moon Jaeyoung */	
 	prompt  ########  3.1 tablespace free space ( monitoring �� 4�� )########
	set linesize 500;
	set feedback off;
...
*/
	prompt
 	prompt  ########  3.2 undo segment usage ( monitoring �� 8�� )########
	set linesize 500;
	set feedback off;
	...
	prompt
	prompt  ########4. DISK I/O ########
	prompt  ########  4.1 File I/O ( monitoring �� 5�� )########
	set linesize 500;
	set feedback off;
...
/	
  ...
  FROM
        ...
        COUNT(*) acs  FROM v$session WHERE status='READY') a ,
	...
	!ps -ef | grep tbsvr | grep -v grep | grep $TB_SID
	prompt #### TOTAL Count ####
	!ps -ef | grep tbsvr | grep -v grep | grep $TB_SID | wc -l
	...
	prompt
	prompt ########8. Alert Log########
	prompt ########  8.1 callstack �߻� ########
	!find $TB_HOME/instance/$TB_SID/* -name "tbsvr.*" -print | xargs ls -alt


	prompt
	prompt ########9. ETC########
	prompt ########  9.1 CANT_EXTEND or OUT_OF_SHP �߻����� ########
/* ��ü �˻�  */
--	!egrep -n 'ERROR_TX_CANT_ALLOC_EXT|ERROR_OUT_OF_SHP' $TB_HOME/instance/$TB_SID/log/slog/*
--�α����� ��ġ�� �ٸ��� ���
--	!egrep -n 'ERROR_TX_CANT_ALLOC_EXT|ERROR_OUT_OF_SHP' `cat $TB_HOME/config/$TB_SID.tip | grep SLOG_DEST | cut -d'"' -f2 | cut -d'=' -f2`/*

/* �������� 60�� �̳��� ���ϸ� �˻� */
	!find $TB_HOME/instance/$TB_SID/log/slog/* -type f -mtime -60 | xargs egrep -n 'ERROR_TX_CANT_ALLOC_EXT|ERROR_OUT_OF_SHP'
--�α����� ��ġ�� �ٸ��� ���
--	!find `cat $TB_HOME/config/$TB_SID.tip | grep SLOG_DEST | cut -d'"' -f2 | cut -d'=' -f2`/* -type f -mtime -60 | xargs egrep -n 'ERROR_TX_CANT_ALLOC_EXT|ERROR_OUT_OF_SHP'
